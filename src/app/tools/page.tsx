      ...
      {activeTab === "tqs" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="mb-5">
                ...
                <input type="range" min={5} max={95} value={bouncePct} onChange={(e) => setBouncePct(Number(e.target.value))} className="w-full accent-[#667eea]" />
                <div className="flex justify-between text-xs text-gray-400"><span>5%</span><span>95%</span></div>
              </div>

              <div className="mb-5">
                <div className="flex justify-between text-sm mb-1">
                  <span className="font-medium text-gray-700">Sayfa/Ziyaret</span>
                  <span className="text-[#667eea] font-semibold">{pagesPerVisit.toFixed(1)}</span>
                </div>
                <input type="range" min={1} max={15} step={0.5} value={pagesPerVisit} onChange={(e) => setPagesPerVisit(Number(e.target.value))} className="w-full accent-[#667eea]" />
              ...
              <div className="mb-5">
                ...
                <div className="flex justify-between text-xs text-gray-400"><span>10sn</span><span>15dk</span></div>
              </div>

              <div className="mb-5">
                <label className="block text-sm font-medium text-gray-700 mb-1">AOV ($)</label>
                <input
                  type="number"
                  value={tqsAov}
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h2 className="font-semibold text-gray-900 mb-4">TQS (Traffic Quality Score)</h2>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="py-2 px-3 text-left text-gray-500 font-medium">TQS</th>
                      {Array.from({ length: 10 }, (_, i) => i + 1).map((t) => (
                        <th key={t} className={`py-2 px-3 text-center font-medium ${Math.round(tqs) === t ? "text-[#667eea]" : "text-gray-500"}`}>{t}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="mb-5">
                <div className="flex justify-between text-sm mb-1">
                  <span className="font-medium text-gray-700">Ortalama Bundle Boyutu</span>
                  <span className="text-[#667eea] font-semibold">{bundleSize.toFixed(1)}x</span>
                </div>
                <input type="range" min={1} max={5} step={0.1} value={bundleSize} onChange={(e) => setBundleSize(Number(e.target.value))} className="w-full accent-[#667eea]" />
                <div className="flex justify-between text-xs text-gray-400"><span>1x</span><span>5x</span></div>
              </div>
              ...
              <div className="mb-2">
                ...
                <div className="flex justify-between text-xs text-gray-400"><span>0%</span><span>50%</span></div>
              </div>
            </div>
          </div>
                  ...
                  return (
                    <div key={seg.label}>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className={`font-medium ${isMatch ? "text-[#667eea]" : "text-gray-500"}`}>{formatMoney(seg.value)}</span>
                      </div>
                ...
                <table className="w-full text-sm table-striped">
                  ...
                  <tbody>
                    {Object.entries(NICHE_AOV).map(([key, val]) => (
                      <tr key={key} className={`border-b border-gray-100 ${key === aovNiche ? "bg-[#667eea]/5" : ""}`}>
                        <td className="py-2 px-3 font-medium text-gray-700">{AOV_NICHE_LABELS[key]}</td>
                        <td className="py-2 px-3 text-right text-gray-600">{formatMoney(val.low)}</td>
                        <td className="py-2 px-3 text-right text-gray-600">{formatMoney(val.mid)}</td>
